* Each {{process}} has its own virtual address space, while all processes share the same {{real (physical) memory}}.
    ...
    * What does it mean to say that a process has "the illusion of dedicated memory"? {{Because each process has its own mapping, it appears to have private, contiguous memory. But the underlying physical memory is actually shared with other processes & the OS}}.
    * Some regions are kernel‑mode only. Accessing them in {{user}} mode triggers an exception.
...
* The stack grows {{down}}wards in memory.
